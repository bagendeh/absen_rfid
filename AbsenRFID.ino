...
const char* password = "mautaubanget";

//pengenal host (server) = IP Address komputer server
const char* host = "192.168.1.248";
...
void loop() {
  ...
  if(digitalRead(BTN_PIN)==1)   //ditekan
  {
     ...
     HTTPClient http ;
     //Get Data
     Link = "http://192.168.1.248/absen/ubahmode.php";
     http.begin(Link);
  ...
  String Link;
  HTTPClient http;
  Link = "http://192.168.1.248/absen/kirimkartu.php?nokartu=" + IDTAG;
  http.begin(Link);
